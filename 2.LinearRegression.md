...
$$
\begin{align}
\hat{w}=\mathop{argmax}\limits_wp(w|Y, X)=\mathop{argmax}\limits_w\frac{p(Y|w, X)p(w)}{p(Y|X)}
\end{align}
$$
忽略似然概率$p(Y|X)$，因为似然概率和 $w$ 没有关系，可以看作常数。
$$
\begin{align}
p(Y|w, X)p(w)&=\log p(Y|w,X)+\log p(w)\\
&=\log(\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(Y-w^TX)(Y-w^TX)^T}{2\sigma^2}})+\log(\frac{1}{\sqrt{2\pi}\sigma_0}e^{-\frac{w^Tw}{2\sigma_0^2}})\\
&=\log\frac{1}{\sqrt{2\pi}\sigma}-\frac{(y-w^Tx)(y-w^Tx)^T}{2\sigma^2}\log e+\log\frac{1}{\sqrt{2\pi}\sigma_0}-\frac{w^Tw}{2\sigma_0^2}\log e\\
&\Longrightarrow \mathop{argmax}\limits_wp(w|Y, X)\propto\mathop{argmin}\limits_w[(Y-w^TX)(Y-w^TX)^T+\frac{\sigma^2}{\sigma_0^2}w^Tw]
\end{align}
$$
